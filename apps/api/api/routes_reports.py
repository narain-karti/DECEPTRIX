...
def get_report_pdf(job_id: str, db: Session = Depends(get_db)):
    ...
    pdf.cell(200, 10, f"DECEPTRIX Audit Report", ln=True, align='C')
    
    pdf.set_font("Helvetica", size=12)
    pdf.cell(200, 10, f"Job ID: {job.id}", ln=True)
    pdf.cell(200, 10, f"Modality: {job.modality}", ln=True)
    pdf.cell(200, 10, f"Status: {job.status}", ln=True)
    pdf.cell(200, 10, f"Verdict: {job.verdict}", ln=True)
    
    pdf.ln(10)
    pdf.set_font("Helvetica", style="B", size=14)
    pdf.cell(200, 10, "Evidence Timeline", ln=True)
    ...
    if job.evidence:
        for ev in job.evidence:
            ...
            pdf.multi_cell(0, 10, f"- {typ}: {expl} (Score: {score})")
            ...
            if refs:
                for ref in refs:
                    ...
                    faces = ref.get("faces", [])
                    if ts is not None:
                        pdf.cell(0, 5, f"   Frame {ts}s - Faces detected: {len(faces)}", ln=True)
                        for face in faces:
                            face_crop_path = face.get("face_crop")
                            fake_score = face.get("fake_score", 0)
                            if face_crop_path and os.path.exists(face_crop_path):
                                pdf.cell(0, 5, f"      Face Deepfake Score: {fake_score:.2f}", ln=True)
                                try:
                                    pdf.image(face_crop_path, w=30)
                                    pdf.ln(2)
                                except Exception as e:
                                    pdf.cell(0, 5, f"      [Failed to embed image]", ln=True)
            pdf.ln(2)
    else:
        pdf.cell(200, 10, "No evidence collected.", ln=True)
